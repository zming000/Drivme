...
public class UserWelcomeBack extends AppCompatActivity {
    //declare variables
    TextView mtvHello;
        ...
        handler.postDelayed(() -> {
            if(getRole != null) {
                if(getRole.equals("Tourist")) {
                    //go to car details
                    startActivity(new Intent(UserWelcomeBack.this, TouristNavHomepage.class));
                }
                else{
                    //go to driving details
                    startActivity(new Intent(UserWelcomeBack.this, DriverNavHomepage.class));
                }
            }
            finishAffinity();
            finish();
